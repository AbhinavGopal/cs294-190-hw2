...
prior_success = np.array([1 for a in range(n_actions)])
prior_failure = np.array([1 for a in range(n_actions)])
...
def choose_action(state_idx, epsilon, use_ts):
    if np.random.random() < 1 - epsilon:
        return np.argmax(q_table[state_idx])
    else:
        if use_ts:
            return choose_action_ts()
        else:
            return choose_action_random()
...
def choose_action_ts():
    sampled_means = np.random.beta(prior_success, prior_failure)
    action = random_argmax(sampled_means)
    return action

def main(epsilon, use_ts):
    ...
    for episode in range(30000):
        ...
        while True:
            ...
            action = choose_action(state_idx, epsilon, use_ts)
            next_state, reward, done, _ = env.step(action)
            
            irl_reward = get_reward(feature_matrix, theta, n_states, state_idx)

            if irl_reward > -0.5:
                prior_success[action] += 1
            else:
                prior_failure[action] += 1
...
if __name__ == '__main__':
    ...
    main(args.epsilon, args.epsilon_ts)